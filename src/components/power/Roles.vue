      ...
      <el-table :data="roleList" border stripe>
            ...
            <el-row :class="['bdbottom',i1===0? 'bdtop': '','vcenter']" v-for="(item1,i1) in scope.row.children" :key="item1.id">
              ...
              <el-col :span="19">
                  <el-row :class="[i2===0? '': 'bdtop','vcenter']" v-for="(item2,i2) in item1.children" :key="item2.id">
                      <!-- 二級權限 -->
                      <el-col :span="8">
                          <el-tag type="success" closable @close="removeRightsById(item2.id)">{{item2.authName}}</el-tag>   
                          <i class="el-icon-caret-right"></i>                       
                      </el-col>
                      <!-- 三級權限 -->
                      <el-col :span="16">
                          <el-tag type="warning"  v-for="(item3,i3) in item2.children" :key="item3.id" closable @close="removeRightsById(item3.id)">
                              {{item3.authName}}
                          </el-tag>                         
                      </el-col>
                  </el-row>         
        ...
        <el-table-column label="功能" width="300px">
          <template slot-scope="scope">
            ...
            <el-button type="warning" icon="el-icon-setting" size="mini">分配權限</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
